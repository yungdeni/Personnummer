...
namespace Personnummer
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (userInput.Length == 11)
            {
                if (int.TryParse(userInput.Substring(0, 6), out int yearCheck))
                {
                    ...
                    {
                        if (CompareToCurrentDate(userInput.Substring(0, 6)))
                        {
        ...
        static bool CompareToCurrentDate(string userInputDate)
        {
            ...
            //this means we should subtract a century if there is a + sign instead of - in personnummer
            if ((date.Year-2000) >= userYear && date.Month >= userMonth && date.Day > userDay)
            {
                return true;
            }
            return false;
